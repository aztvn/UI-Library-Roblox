...
local function Hide(Window, bind, notif,closexxx)
	...
	bind = bind[2]
	if notif then
		CSWZ:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	CSWZUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(CSWZUI)
	CSWZUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	CSWZUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == CSWZUI.Name and Interface ~= CSWZUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "CSWZ-Old"
		end
	end
elseif not isStudio then
...
CSWZUI.Enabled = false
CSWZUI.SmartWindow.Visible = false
CSWZUI.Notifications.Template.Visible = false
CSWZUI.DisplayOrder = 1000000000

local Main : Frame = CSWZUI.SmartWindow
local Dragger = Main.Drag
local dragBar = CSWZUI.Drag
...
local Notifications = CSWZUI.Notifications
...
-- 	for FlagName, Flag in pairs(CSWZ.Flags) do
...
-- 		else
-- 			notified = true
-- 			CSWZ:Notification({Title = "Config Error", Content = "CSWZ was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		CSWZ:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using CSWZ Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		CSWZ:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using CSWZ Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(CSWZ.Flags) do
...
function CSWZ:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function CSWZ:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "CSWZ UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
...
function ButtonZZZZZ:NewCreateButton(bind,Window)
	...
	Main.Logo.Image = WindowSettings.LogoID 
	Main.Visible = true
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "CSWZ Interface Suite" and Release or "CSWZ UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(CSWZUI.MobileSupport, CSWZUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "CSWZ/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "CSWZ/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".CSWZ") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".CSWZ"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".CSWZ", FoundKey)
						end
						CSWZ:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "CSWZ Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("CSWZ Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("CSWZ Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- CSWZ.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					CSWZ.Options[Flag] = SliderV
				end

				CSWZUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = CSWZ.ThemeGradient
					Slider.Main.UIStroke.color.Color = CSWZ.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("CSWZ Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				CSWZUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = CSWZ.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = CSWZ.ThemeGradient
				end)

				if Flag then
					CSWZ.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("CSWZ Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					CSWZ.Options[Flag] = BindV
				end

				-- CSWZ.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("CSWZ Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("CSWZ Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- CSWZ.Flags[DropdownSettings.Flag] = DropdownSettings
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("CSWZ Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					CSWZ.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = CSWZ:SaveConfig(inputPath)
					if not success then
						CSWZ:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = CSWZ:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = CSWZ:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = CSWZ:LoadConfig(selectedConfig)
					if not success then
						CSWZ:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = CSWZ:SaveConfig(selectedConfig)
					if not success then
						CSWZ:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(CSWZ.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(CSWZ.Folder .. "/settings/autoload.txt")
			...
			if isfile(CSWZ.Folder .. "/settings/autoload.txt") then
				local name = readfile(CSWZ.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if CSWZ.Options[Flag] then
						CSWZ.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if CSWZ.Options[Flag] and data.value then
						CSWZ.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if CSWZ.Options[Flag] and data.text and type(data.text) == "string" then
						CSWZ.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if CSWZ.Options[Flag] and data.value then
						CSWZ.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if CSWZ.Options[Flag] and data.color then
						CSWZ.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "CSWZInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "CSWZInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "CSWZInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						CSWZ.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						CSWZUI.ThemeRemote.Value = not CSWZUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						CSWZ.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						CSWZUI.ThemeRemote.Value = not CSWZUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						CSWZ.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						CSWZUI.ThemeRemote.Value = not CSWZUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				CSWZ.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				CSWZ.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function CSWZ:SaveConfig(Path)
			...
			local fullPath = CSWZ.Folder .. "/settings/" .. Path .. ".CSWZ"

			local data = {
				objects = {}
			}

			for flag, option in next, CSWZ.Options do
		...
		function CSWZ:LoadConfig(Path)
			...
			local file = CSWZ.Folder .. "/settings/" .. Path .. ".CSWZ"
		...
		function CSWZ:LoadAutoloadConfig()
			if isfile(CSWZ.Folder .. "/settings/autoload.txt") then
				...
				local success, err = CSWZ:LoadConfig(name)
				if not success then
					return CSWZ:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function CSWZ:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(CSWZ.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".CSWZ" then
					local pos = file:find(".CSWZ", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			CSWZUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			CSWZUI.MobileSupport.Visible = false
	...
	CSWZUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		CSWZUI.MobileSupport.Visible = false
...
if isStudio then
	...
	CSWZ:Notification({ 
		Title = "Welcome to CSWZ",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the CSWZ Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. CSWZ was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. CSWZ Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using CSWZ Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "CSWZ's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = CSWZ:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the CSWZ Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. CSWZ was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. CSWZ Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using CSWZ Today at " .. website})
